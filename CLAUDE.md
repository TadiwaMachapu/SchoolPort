...
- **Overall-average scope: Term Report vs at-risk (Sprint 1.5.3).** BOTH now read the ONE captured-marks path (`AtRiskMarks.CapturedPredicate` — Term Report was migrated off the legacy Submission-join path, which couldn't see submission-less bulk-captured marks). They still differ by TIME SCOPE **by design**: the **Term Report** averages the SELECTED TERM (a formal term document); the **at-risk / Matric triage** average (`AtRiskService`) spans ALL past-due captured marks (current live standing, term used only for trend). Same marks source, different window — do NOT "reconcile" the two numbers; they answer different questions. (If a future sprint wants them labelled distinctly in the UI, that's the deferred relabel.)
