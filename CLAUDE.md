...
- **Token set lacks an informational tone (token-set gap — NOT a semantic scale).** The palette has `primary`, `secondary`, `danger`, `warning`, `success` — but no neutral-informational tone. States like "graded", "submitted", "draft" aren't success/warning/danger, they're just states; with no info tone they either collapse to green-on-green (submitted=success next to graded, indistinguishable) or stay un-migrated. Symptom found during the rollout: `AssignmentsTab.StatusBadge` (graded=blue has no token) was left deferred for exactly this reason. **Add an `info-100/500/700` family (blue-based, distinct from `primary` and from the status hues) for neutral state badges**, then migrate the deferred state badges to it. Resolve alongside the semantic-scale pass. (Also unblocks `assignments/[id]` STATUS_COLORS consistency and any other graded/submitted/draft badge.)
- **Decorative gradients need a design pass (NOT a colour-token migration).** `courses.GRADIENTS` (the 6 rotating course-thumbnail gradients) and any other decorative gradients can't be mechanically swapped: the design brief bans gradients, and removing one needs a *replacement decision* — flat tint, subject-coded colour, or icon treatment? Left hardcoded during the rollout. Scope a small design pass to decide the replacement and apply it (its own verification, not the colour rollout).
- **Chart theming pass (a DIFFERENT CATEGORY from the token migration — colour-as-prop, not colour-as-class).** Chart libraries take colour as **props with literal values**, not as Tailwind utility classes — so they are **structurally invisible to any utility-based colour migration** and will NEVER be caught by a grep for `bg-*`/`text-*`. Found during the rollout: `analytics/page.tsx` recharts (`GRADE_COLORS` hex array + `<Pie>`/`<Bar>` `fill="#3b82f6"` etc.). **Consequence for branding: a school that changes their primary colour sees the whole app follow EXCEPT the charts, which stay hardcoded.** recharts accepts CSS variables, so the fix is to feed chart props from the token CSS vars (`var(--color-primary)`, etc.) instead of literals. **Its own pass: audit EVERY chart in the app (analytics, dashboards, any recharts usage) — not just the ones found incidentally during this rollout.** ⚠️ The same blind spot applies to **anything else that consumes colour as a prop rather than a class** (other chart libs, canvas/SVG code, inline `style={{ color }}`, map/vis libraries) — the audit should sweep for colour-as-prop generally, not only recharts.
