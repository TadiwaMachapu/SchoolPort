...
- **`submissions` storage bucket — RESOLVED (Sprint 1.5.0.6, applied live 2026-07-05).** The bucket is now **private** (`migrations/007_private_submissions.sql`): unsigned URLs return `400 Bucket not found` (verified live), and files are served only via short-lived signed URLs (1-hour expiry) minted server-side by `StorageService` with the service role key at DTO-build time. `Submission.FileUrl` stores the bucket-relative object path (legacy public-URL rows are normalised on read); a signing failure yields `null` in the DTO, never a raw path.
