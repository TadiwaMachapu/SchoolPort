...
- **Branding write-path bug** — promoted to its own decision note: [[Branding Write-Path Bug]] (verify the `school.manage` → SchoolsController theme write takes effect end-to-end; resolve the unconsumed `SchoolTheme.FontFamily`). Tracked there, not here.
- **At-risk query performance** — promoted to its own decision note: [[At-Risk Query Performance]] (profile `AtRiskService.EvaluateAsync` at realistic 500–1200-learner scale before pilot, not the 3-learner [[Greendale High School (Demo)|Greendale]] demo). Tracked there, not here.
