    ...
    return (
      <div className="p-6 lg:p-8">
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <h3 className="mt-4 text-base font-semibold text-gray-900">Not permitted</h3>
          <p className="mt-1 text-sm text-gray-500">Managing positions requires the Positions Assign permission (Principal, Deputy Principal, or IT Administrator).</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 lg:p-8">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-gray-900">Positions</h1>
          <p className="mt-1 text-sm text-gray-500">Who holds what across your school — assign, scope, and revoke staff positions.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="mt-6 flex flex-col items-center justify-center rounded-xl border border-gray-100 bg-white py-16 text-center shadow-sm ring-1 ring-gray-100/50">
          <IdCard className="h-10 w-10 text-gray-300" />
          <h3 className="mt-4 text-base font-semibold text-gray-900">No positions assigned yet</h3>
          <p className="mt-1 text-sm text-gray-500 max-w-sm">Assign a position to a staff member, or bulk-import staff with positions from the onboarding wizard.</p>
        ...
        <>
          ...
          <div className="mt-3 space-y-4">
            {overview.map((p) => (
              <div key={p.positionKey} className="rounded-xl border border-gray-100 bg-white shadow-sm ring-1 ring-gray-100/50">
                <div className="flex items-center justify-between border-b border-gray-100 px-5 py-3">
                  <div className="flex items-center gap-2">
                    <h2 className="text-sm font-semibold text-gray-900">{p.displayName}</h2>
                    <Badge variant="outline" className="text-[10px]">{p.category}</Badge>
                  </div>
                  <span className="flex items-center gap-1 text-xs text-gray-400"><UsersIcon className="h-3.5 w-3.5" />{p.holders.length}</span>
                </div>
                <div className="divide-y divide-gray-50">
                  {p.holders.map((h) => (
                    <div key={h.userPositionId} className="flex items-center justify-between gap-3 px-5 py-3">
                      <div className="min-w-0">
                        <p className="truncate text-sm font-medium text-gray-900">{h.userName}</p>
                        <div className="mt-1 flex flex-wrap items-center gap-1.5">
                          {h.scopes.length === 0
                            ? <span className="text-xs text-gray-400">School-wide / unscoped</span>
                            : h.scopes.map((s, i) => <Badge key={i} variant="default" className="text-[10px]">{s.label}</Badge>)}
                          <span className="text-xs text-gray-400">· {fmt(h.effectiveFrom)}{h.effectiveTo ? ` → ${fmt(h.effectiveTo)}` : ""}</span>
                        </div>
                      </div>
                      <Button variant="ghost" size="sm" className="shrink-0 text-rose-600 hover:bg-rose-50"
