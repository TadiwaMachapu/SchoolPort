...
function AdminView() {
  ...
  if (loading) return <div className="flex items-center justify-center h-64"><Loader2 className="h-6 w-6 animate-spin text-text-muted" /></div>;

  return (
    <div className="space-y-6">
      {error && <div className="rounded-lg bg-danger-100 px-4 py-3 text-sm text-danger-700">{error}</div>}

      {/* Add fee form */}
      {showForm ? (
        <div className="rounded-2xl bg-surface-card border border-border shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-border flex items-center justify-between">
            <h2 className="text-base font-semibold text-text-primary">New fee</h2>
            <button onClick={() => setShowForm(false)}><X className="h-4 w-4 text-text-muted" /></button>
          </div>
          <div className="px-6 py-5 space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-text-primary">Amount (ZAR) <span className="text-danger-500">*</span></label>
                <Input type="number" min={0} step={0.01} value={form.amountZar} onChange={e => setForm(f => ({ ...f, amountZar: e.target.value }))} placeholder="0.00" />
              </div>
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-text-primary">Due date <span className="text-danger-500">*</span></label>
                <Input type="date" value={form.dueDate} onChange={e => setForm(f => ({ ...f, dueDate: e.target.value }))} />
              </div>
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-text-primary">Term (optional)</label>
            ...
            <div className="flex items-center gap-3 pt-1">
              ...
              <button onClick={() => setShowForm(false)} className="text-sm text-text-secondary hover:text-text-primary">Cancel</button>
            </div>
          </div>
        </div>
      ) : canCreateInvoice ? (
      ...
      {payForm && (
        <div className="rounded-2xl bg-surface-card border border-primary-200 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-border flex items-center justify-between">
            <h2 className="text-base font-semibold text-text-primary">Record payment</h2>
            <button onClick={() => setPayForm(null)}><X className="h-4 w-4 text-text-muted" /></button>
          </div>
          <div className="px-6 py-5 space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-text-primary">Learner student number</label>
                <Input value={payForm.studentNumber} onChange={e => setPayForm(p => p && ({ ...p, studentNumber: e.target.value }))} placeholder="STU2026001" />
              </div>
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-text-primary">Amount paid (ZAR)</label>
            ...
            <div className="flex items-center gap-3 pt-1">
              ...
              <button onClick={() => setPayForm(null)} className="text-sm text-text-secondary hover:text-text-primary">Cancel</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="rounded-2xl bg-surface-card border border-border shadow-sm overflow-hidden">
        {fees.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-sm font-medium text-text-secondary">No fees configured</p>
            <p className="text-xs text-text-muted mt-1">Add a fee to start tracking payments.</p>
          </div>
        ) : fees.map(fee => (
          <div key={fee.feeId} className="border-b border-border last:border-0">
            <div className="flex items-center gap-4 px-5 py-4 hover:bg-surface-subtle">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-text-primary">{fee.name}</p>
              ...
              <div className="text-right shrink-0">
                <p className="text-sm font-bold text-text-primary">{fmt(fee.amountZar)}</p>
                <p className="text-xs text-success-700">{fmt(fee.totalCollected)} collected · {fee.paymentCount} payment{fee.paymentCount !== 1 ? "s" : ""}</p>
              </div>
              {canRecordPayment && (
                <button onClick={() => setPayForm({ feeId: fee.feeId, studentNumber: "", amount: "", notes: "" })}
                  className="shrink-0 rounded-lg border border-border bg-surface-card px-3 py-1.5 text-xs font-medium text-text-primary hover:bg-surface-subtle transition-colors">
                  + Payment
                </button>
              )}
              <button onClick={() => deleteFee(fee.feeId)}
                className="h-7 w-7 rounded flex items-center justify-center text-text-muted hover:text-danger-500 hover:bg-danger-100 transition-colors shrink-0">
                <Trash2 className="h-3.5 w-3.5" />
              </button>
            </div>

            {expanded === fee.feeId && (
              <div className="px-5 pb-4 bg-surface-subtle border-t border-border">
                {!payments[fee.feeId] ? (
                  <p className="text-xs text-text-muted py-3">Loading…</p>
                ) : payments[fee.feeId].length === 0 ? (
                  <p className="text-xs text-text-muted py-3">No payments recorded yet.</p>
                ) : (
                    ...
                    <tbody className="divide-y divide-border">
                      {payments[fee.feeId].map(p => (
                        <tr key={p.feePaymentId}>
                          <td className="py-1.5 text-text-primary">{p.studentName} <span className="text-text-muted">({p.studentNumber})</span></td>
                          <td className="py-1.5 font-semibold text-success-700">{fmt(p.amountPaidZar)}</td>
                          <td className="py-1.5 text-text-secondary">{fmtDate(p.paidAt)}</td>
                          <td className="py-1.5 text-text-muted">{p.notes ?? "—"}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: "Total Fees", value: fmt(statement.reduce((s, f) => s + f.amountZar, 0)), color: "text-text-primary" },
          { label: "Total Paid", value: fmt(totalPaid), color: "text-success-700" },
          { label: "Outstanding", value: fmt(Math.max(0, totalOwed)), color: totalOwed > 0 ? "text-danger-700" : "text-text-muted" },
        ].map(s => (
          <div key={s.label} className="rounded-xl bg-surface-card border border-border p-4 text-center shadow-sm">
      ...
      <div className="rounded-2xl bg-surface-card border border-border shadow-sm overflow-hidden">
        {statement.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <CreditCard className="h-10 w-10 text-text-muted mb-3" />
            <p className="text-sm text-text-secondary">No fees have been issued for your account.</p>
          </div>
        ) : (
          <>
            <div className="grid grid-cols-[1fr_100px_100px_100px_80px] gap-2 px-5 py-2.5 border-b border-border text-xs font-semibold text-text-muted uppercase tracking-wider">
            ...
            {statement.map(f => (
              <div key={f.feeId} className="grid grid-cols-[1fr_100px_100px_100px_80px] gap-2 items-center px-5 py-3.5 border-b border-border last:border-0 hover:bg-surface-subtle">
                <div>
                  <p className="text-sm font-medium text-text-primary">{f.name}</p>
                  <p className="text-xs text-text-muted">Due {fmtDate(f.dueDate)}{f.description ? ` · ${f.description}` : ""}</p>
                </div>
                <p className="text-sm text-right text-text-primary">{fmt(f.amountZar)}</p>
                <p className="text-sm text-right text-success-700 font-medium">{fmt(f.amountPaid)}</p>
                <p className={`text-sm text-right font-semibold ${f.balance > 0 ? "text-danger-700" : "text-text-muted"}`}>{fmt(Math.max(0, f.balance))}</p>
                <div className="flex justify-end">
                  {f.isPaid
                    ? <span className="text-xs font-medium text-success-700 bg-success-100 rounded-full px-2 py-0.5">Paid</span>
                    : <span className="text-xs font-medium text-danger-700 bg-danger-100 rounded-full px-2 py-0.5">Owing</span>
                  }
                </div>
              </div>
            ))}
...
export default function SchoolPayPage() {
  ...
  if (!hasFlag) {
    return (
      <div className="flex flex-col items-center justify-center h-96 text-center px-4">
        ...
        <h2 className="text-lg font-semibold text-text-primary">SchoolPay not enabled</h2>
        <p className="text-sm text-text-muted mt-1">Enable the SchoolPay feature in Settings.</p>
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 max-w-4xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-text-primary tracking-tight">SchoolPay</h1>
