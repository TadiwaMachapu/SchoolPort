    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-0 backdrop-blur-sm sm:items-center sm:p-4" onClick={onClose}>
      <div className="w-full max-w-lg rounded-t-2xl bg-white shadow-2xl sm:rounded-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4">
          <h3 className="text-base font-semibold text-gray-900">Bulk import staff</h3>
          <button onClick={onClose} className="rounded-md p-1 text-gray-400 hover:bg-gray-100"><X className="h-5 w-5" /></button>
        </div>

        <div className="space-y-4 px-5 py-4">
          <p className="text-sm text-gray-500">
            CSV columns: <code className="rounded bg-gray-100 px-1 text-xs">name,email,identity,positions,scopes</code>.
            Positions are <code className="rounded bg-gray-100 px-1 text-xs">;</code>-separated; scopes map a position to its scope,
            e.g. <code className="rounded bg-gray-100 px-1 text-xs">HOD:Mathematics;GradeHead:10</code> (quote the scopes cell — it can contain commas).
          </p>
          ...
          <div className="rounded-lg border-2 border-dashed border-gray-200 p-4 text-center">
            ...
            <button onClick={() => fileRef.current?.click()} className="text-sm font-medium text-blue-600 hover:underline">
